...
def make_graph(graph_fname, chunks, minlink, maxlink, weight):
  ...
  if maxlink > 1:
    filt = lambda x:minlink <= x <= maxlink
  else:
    filt = lambda x:minlink <= x
...
def create_new_corpus(corpus, chunk_cl):
  ...
  for sentence in corpus:
    ...
    for w in sentence:
      if is_chunk(w):
        new_sentence.append(chunk_tag(chunk_cl[w]))
      else:
...
def main():
  ...
  graph_init_fname = opt.graph or graph_initial_fname(opt.input, opt.maxlinking)
